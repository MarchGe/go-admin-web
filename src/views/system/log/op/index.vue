<template>
  <div>
    ...
    <div class="search_head">
      ...
      <el-button type="primary" icon="Search" :disabled="!hasPermission('opLog:delete')" @click="clearLog">清空</el-button>
    </div>
    <!--表格数据-->
    <el-table class="table_content" :data="x.tableData" :stripe="true" empty-text="暂无数据">
      <el-table-column type="expand" width="40px">
        <template #default="props">
          <div class="inner_content">
            <div class="inner_row">
              <span class="label">来源UA：</span>
              <span class="value">{{props.row.userAgent}}</span>
            </div>
            <div class="inner_row">
              <span class="label">请求Path：</span>
              <span class="value">{{props.row.path}}</span>
            </div>
            <div class="inner_row">
              <span class="label">Url参数：</span>
              <span class="value">{{props.row.query ? props.row.query : "无"}}</span>
            </div>
            <div class="inner_row">
              <span class="label">请求体：</span>
              <span class="value">{{props.row.body && props.row.body !== "null" ? props.row.body : "无"}}</span>
            </div>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="动作" prop="action" header-align="center" align="center"></el-table-column>
      <el-table-column label="操作对象" prop="target" header-align="center" align="center"></el-table-column>
      <el-table-column label="操作时间" prop="createTime" header-align="center" align="center" width="200px"></el-table-column>
    </el-table>
...
function pageChanged(page) {
  ...
  httpUtil.get(serverPaths.opLog, params, function (resData) {
    x.currentPage = page
...
function doClearLog() {
  ...
  httpUtil.delete(serverPaths.opLogDelete, params, function () {
...
<style scoped src="@/assets/css/sys/op-log.css">
